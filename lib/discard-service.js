(function registerDiscardService(globalScope) {
  ...
  async function discardTabWithPolicy(
    tabId,
    { tabsApi, tabStateModel, shouldDiscardTab },
  ) {
    ...
    if (shouldDiscardTab && !shouldDiscardTab(tab)) {
      return {
        status: RESULT_STATUSES.SKIPPED,
        tabId,
        tab,
        reason: "no-longer-targeted",
      };
    }
